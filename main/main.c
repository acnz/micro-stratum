...
esp_err_t stats_get_handler(httpd_req_t *req)
{
    ...
    snprintf(buf, buf_len,
             ...
             "<div class='stat'>Dificuldade: <span class='val'>%.8f</span></div>"
...
static void stratum_client_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        // 1. O Subscribe com strlen automático (para nunca mais cortar o \n)
        ...
        // 3. O Pedido de Arrego (Dificuldade Fracionada)
        char suggest[128];
        ...
        while (1)
        {
                        ...
                        if (id && id->valueint == 1)
                        {
                            ...
                            if (cJSON_IsArray(res) && cJSON_GetArraySize(res) >= 3)
                            {
                                ...
                                if (cJSON_IsString(extra))
                                    strcpy(g_extranonce1, extra->valuestring);
                            }
                        }
                        cJSON_Delete(j);
                    }
            ...
            if (uart_read_bytes(UART_PORT, resp, 11, 10 / portTICK_PERIOD_MS) == 11 && resp[0] == 0xAA && resp[1] == 0x55)
            {
                ...
                if (valid)
                {
                    ...
                    char sub[256];
                    snprintf(sub, 256, "{\"id\": 4, \"method\": \"mining.submit\", \"params\": [\"%s.%s\", \"%s\", \"00000000\", \"%s\", \"%08lx\"]}\n", BTC_ADDRESS, WORKER_NAME, g_last_job_id, g_last_ntime, (unsigned long)nonce);
            ...
            { // 30 segundos
                ...
                // MUDANÇA AQUI: %.5f para suportar casas decimais longas!
