...
double g_best_diff = 0.0; // Recorde de dificuldade da sessão
double g_network_difficulty = 0.0;
...
int g_extranonce2_size = 4;          // Tamanho padrão seguro
char g_extranonce2[32] = "00000000"; // Dinâmico de acordo com a Pool!
...
void hex_to_bytes(const char *hex, uint8_t *bytes)
{
    ...
    for (size_t i = 0; i < len / 2; i++)
    {
        ...
        uint8_t h = (high >= '0' && high <= '9') ? (high - '0') : (high >= 'a' && high <= 'f') ? (high - 'a' + 10)
                                                              : (high >= 'A' && high <= 'F')   ? (high - 'A' + 10)
                                                                                               : 0;
        uint8_t l = (low >= '0' && low <= '9') ? (low - '0') : (low >= 'a' && low <= 'f') ? (low - 'a' + 10)
...
esp_err_t stats_get_handler(httpd_req_t *req)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            strcat(history_html, "<li>");
            strcat(history_html, g_last_accepted_times[i]);
            strcat(history_html, "</li>");
        }
    }
...
void process_mining_notify(cJSON *params)
{
    ...
    cJSON *j_cb2 = cJSON_GetArrayItem(params, 3);
    cJSON *j_merkle = cJSON_GetArrayItem(params, 4);
    cJSON *j_ver = cJSON_GetArrayItem(params, 5);
    cJSON *j_nbits = cJSON_GetArrayItem(params, 6);
    cJSON *j_ntime = cJSON_GetArrayItem(params, 7);

    if (!cJSON_IsString(j_job) || !cJSON_IsString(j_prev) || !cJSON_IsString(j_cb1) || !cJSON_IsString(j_cb2) || !cJSON_IsArray(j_merkle))
        return;
    ...
    strcat(coinbase_hex, j_cb1->valuestring);
    strcat(coinbase_hex, g_extranonce1);

    // CORREÇÃO CRÍTICA: Extranonce2 agora é dinâmico e tem o tamanho que a Pool manda!
    strcat(coinbase_hex, g_extranonce2);

    strcat(coinbase_hex, j_cb2->valuestring);
...
static void stratum_client_task(void *pvParameters)
{
    ...
    if (!rx)
    {
        ESP_LOGE(TAG, "Falha crítica de RAM.");
        vTaskDelete(NULL);
    }
    ...
    while (1)
    {
        ...
        char auth[256];
        snprintf(auth, 256, "{\"id\": 2, \"method\": \"mining.authorize\", \"params\": [\"%s.%s\", \"x\"]}\n", BTC_ADDRESS, WORKER_NAME);
        ...
        while (1)
        {
            ...
            {
                ESP_LOGE(TAG, "⚠️ Erro de rede! Reconectando...");
                break;
            }
            else if (len > 0)
            {
                ...
                while ((nl = strchr(rx, '\n')))
                {
                    *nl = '\0';
                    ...
                    if (j)
                    {
                        ...
                        {
                            if (!strcmp(m->valuestring, "mining.notify"))
                                process_mining_notify(cJSON_GetObjectItem(j, "params"));
                            else if (!strcmp(m->valuestring, "mining.set_difficulty"))
                                g_pool_difficulty = cJSON_GetArrayItem(cJSON_GetObjectItem(j, "params"), 0)->valuedouble;
                        }
                        cJSON *id = cJSON_GetObjectItem(j, "id");

                        // CORREÇÃO CRÍTICA: Lendo o extranonce2_size que a Pool exige!
                        if (id && id->valueint == 1)
                        {
                            cJSON *res = cJSON_GetObjectItem(j, "result");
                            if (cJSON_IsArray(res) && cJSON_GetArraySize(res) >= 3)
                            {
                                cJSON *extra1 = cJSON_GetArrayItem(res, 1);
                                cJSON *extra2_size = cJSON_GetArrayItem(res, 2);
                                if (cJSON_IsString(extra1))
                                    strcpy(g_extranonce1, extra1->valuestring);
                                if (cJSON_IsNumber(extra2_size))
                                {
                                    g_extranonce2_size = extra2_size->valueint;
                                    memset(g_extranonce2, 0, sizeof(g_extranonce2));
            ...
            if (uart_read_bytes(UART_PORT, resp, 11, 10 / portTICK_PERIOD_MS) == 11 && resp[0] == 0xAA && resp[1] == 0x55)
            {
                ...
                if (current_hash_diff > g_best_diff)
                {
                    ...
                    ESP_LOGW(TAG, "🏆 NOVO RECORDE: %.4f", g_best_diff);
                }
            ...
            vTaskDelay(10 / portTICK_PERIOD_MS); // O respiro pro Watchdog não surtar!
        }
        close(sock);
    }
}
